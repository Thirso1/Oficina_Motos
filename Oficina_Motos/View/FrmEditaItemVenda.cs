...
{
    public partial class FrmEditaItemVenda : Form
    {
        public FrmEditaItemVenda(Itens_Venda itens_venda)
        {
            InitializeComponent();
        ...
        private void FrmEditaItemVenda_Load(object sender, EventArgs e)
        {
            textDescricao.Text = itens_venda.Descricao;
            textValorUni.Text = itens_venda.Valor_uni.ToString();
            textQtde.Text = itens_venda.Qtde.ToString();
            textDesconto.Text = itens_venda.Desconto.ToString();
            textSubTotal.Text = itens_venda.Sub_total.ToString();

            produto = produtoDb.consultaPorId(itens_venda.Id_produto);
            estoque = estoqueDb.consultaPorId(itens_venda.Id_produto);
            estoque_banco = estoque.Estoque_atual;
            qtde_original = itens_venda.Qtde;
        }
        ...
        private void btnSalvar_Click(object sender, EventArgs e)
        {
            ...
            else if ((estoque_banco + qtde_original) >= nova_qtde) //quando a nova qtde é menor que a qtde_original
            {
                ...
                decimal sub_total = nova_qtde * itens_venda.Valor_uni;

                itens_venda.Qtde = nova_qtde;
                itens_venda.Desconto = Convert.ToDecimal(textDesconto.Text);
                itens_venda.Sub_total = sub_total - itens_venda.Desconto;

                Itens_VendaDb itensDb = new Itens_VendaDb();
                itensDb.atualiza(itens_venda);
